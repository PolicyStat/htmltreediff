...
def test_has_fuzzy_hash_collisions_no_children():
    assert _has_fuzzy_hash_collisions([]) is False


def test_all_distinct_tags_dont_have_collisions():
    assert _has_fuzzy_hash_collisions(get_dom_nodes('<p>a</p><h1>b</h1>')) is False


def test_same_tag_twice_has_collision():
    assert _has_fuzzy_hash_collisions(get_dom_nodes('<p>a</p><p>b</p>')) is True


def test_text_nodes_ignored_in_collision_check():
    assert _has_fuzzy_hash_collisions(get_dom_nodes('hello <p>a</p><h1>b</h1>')) is False
...
def test_group_no_pairs_produces_only_sentinel():
    assert group_consecutive_pairs_into_blocks([], 2, 3) == [(2, 3, 0)]


def test_group_consecutive_pairs_merged_into_one_block():
    assert group_consecutive_pairs_into_blocks([(0, 0), (1, 1)], 2, 2) == [
        (0, 0, 2), (2, 2, 0)]


def test_group_non_consecutive_pairs_become_separate_blocks():
    assert group_consecutive_pairs_into_blocks([(0, 0), (2, 2)], 3, 3) == [
        (0, 0, 1), (2, 2, 1), (3, 3, 0)]
...
def test_fuzzy_match_blocks_empty_old():
    assert fuzzy_match_blocks([], get_dom_nodes('<p>hello</p>')) == [(0, 1, 0)]


def test_fuzzy_match_blocks_empty_new():
    assert fuzzy_match_blocks(get_dom_nodes('<p>hello</p>'), []) == [(1, 0, 0)]
...
def test_fuzzy_match_blocks_unmatched_node_excluded_from_blocks():
    ...
    assert fuzzy_match_blocks(old, new) == [(0, 0, 2), (3, 2, 0)]
